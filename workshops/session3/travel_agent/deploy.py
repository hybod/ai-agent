...
async def main():
    ...
    cloud_app = engine.deploy(
        ...
        application_name="travel-agent-v1",
        gateway_name="dong-mcp-agent",
        gateway_service_name="",
        gateway_upstream_name="",
        use_adk_web=False,
        local_test=False,  # Set to True for local testing before deploy to VeFaaS
    )
    # cloud_app = engine.update_function_code(
    #     application_name="travel-agent-v2",  # 现有应用名称
    #     path=str(Path(__file__).parent / "src")        # 本地项目路径
    # )
